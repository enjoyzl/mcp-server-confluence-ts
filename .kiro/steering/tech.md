...
- **HTML 解析**: jsdom (用于 Node.js 环境的 DOM 操作)
- **环境配置**: dotenv 16.4.5
...
### 宏处理配置
项目支持通过 `.macro-config.json` 文件配置宏处理行为：
- **启用/禁用处理器**: 控制特定宏处理器的启用状态
- **黑名单配置**: 完全忽略特定类型的宏
- **回退策略**: 配置宏处理失败时的处理方式
- **处理器设置**: 为每个宏处理器配置特定参数
- **性能参数**: 超时时间、递归深度、并发控制等

## 架构特点
- **模块化设计**: 使用 ES modules
- **严格类型检查**: TypeScript 严格模式
- **配置驱动**: 基于环境变量和配置文件的灵活配置
- **插件化宏处理**: 可扩展的宏处理器架构，支持动态注册
- **跨环境兼容**: HTML 解析器适配器支持 Node.js 和浏览器环境
- **错误恢复机制**: 完善的回退策略和错误处理
- **测试友好**: 完整的单元测试和集成测试支持
